...
public class HttpClientUtil {
	 ...
	 * 特定业务重写的
	 */
	public static String post2(String url, String jsonParams, String encoding){
		String result = "";
		if (StringUtils.isEmpty(url)) {
			log.info("----->url为空");
			return result;
		...
		try {
			...
			httpPost.setHeader("Content-Type","application/json");
			httpPost.setEntity(new StringEntity(jsonParams, ContentType.create("application/json", encoding)));
			httpPost.addHeader("token","FLKJkdsjflkiukjdsj127hflJSLL");
			// 发送请求，并接收响应
			response = httpClient.execute(httpPost);
			result = handleResponse(url, encoding, response);
		} catch (IOException e) {
	...
	*/  
	...
	public static void main(String[] args) {
		String result = get("http://39.105.130.231:8080/practice/practiceStudentRecord/checkIsInProjectByStudentId?studentId=6666");
